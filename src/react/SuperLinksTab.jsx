    ...
    <div className="super-links-tab p-4" role="region" aria-labelledby="superlinks-heading">
      ...
      <form aria-label={t('superLinks') + ' ' + t('settings')} autoComplete="off">
        ...
        <div className="mb-3 row align-items-center">
          <label className="col-md-4 col-form-label fw-bold">{t('queryTimeout', 'Query Timeout (ms):')}</label>
          <div className="col-md-8">
            <input
              className="form-control form-control-sm d-inline-block"
              style={{ maxWidth: 120, display: 'inline-block' }}
              value={get('ext.osds.super_links.timeout') || ''}
              onChange={e => set('ext.osds.super_links.timeout', e.target.value)}
              type="number"
              aria-label={t('queryTimeout', 'Query Timeout')}
              aria-required="false"
            />
            <span className="ms-2">{t('valuesLessThan1000Ignored', '(values less than 1000 are ignored)')}</span>
          </div>
        </div>
        <div className="mb-3 row align-items-center">
          <label className="col-md-4 col-form-label fw-bold">{t('spongeMode', 'Sponge Mode:')}</label>
          <div className="col-md-8 d-flex align-items-center">
            <select
              className="form-select form-select-sm me-2"
              style={{ maxWidth: 200, display: 'inline-block' }}
              value={get('ext.osds.super_links.sponge') || 'describe-ssl'}
              onChange={e => set('ext.osds.super_links.sponge', e.target.value)}
              aria-label={t('spongeMode', 'Sponge Mode')}
              aria-required="false"
            >
              <option value="describe">{t('uriburnerDescribe', 'URIBurner Describe')}</option>
              <option value="describe-ssl">{t('uriburnerDescribeTLS', 'URIBurner Describe TLS')}</option>
              <option value="about">{t('uriburnerAbout', 'URIBurner About')}</option>
              <option value="about-ssl">{t('uriburnerAboutTLS', 'URIBurner About TLS')}</option>
            </select>
            <select
              className="form-select form-select-sm"
              style={{ maxWidth: 200, display: 'inline-block' }}
              value={get('ext.osds.super_links.sponge_mode') || 'xxx'}
              onChange={e => set('ext.osds.super_links.sponge_mode', e.target.value)}
              aria-label={t('spongeModeOption', 'Sponge Mode Option')}
              aria-required="false"
            >
              <option value="xxx">{t('useLocalCache', 'Use local Cache')}</option>
              <option value="soft">{t('automaticCacheInvalidation', 'Automatic Cache Invalidation')}</option>
              <option value="add">{t('addToCache', 'Add to Cache')}</option>
              <option value="replace">{t('forceCacheInvalidation', 'Force Cache Invalidation')}</option>
            </select>
          </div>
        </div>
        <div className="mb-3 row align-items-center">
          <label className="col-md-4 col-form-label fw-bold">{t('viewer', 'Viewer:')}</label>
          <div className="col-md-8">
            <select
              className="form-select form-select-sm"
              style={{ maxWidth: 200, display: 'inline-block' }}
              value={get('ext.osds.super_links.viewer') || 'html-fb'}
              onChange={e => set('ext.osds.super_links.viewer', e.target.value)}
              aria-label={t('viewer', 'Viewer')}
              aria-required="false"
            >
              <option value="html">{t('html', 'HTML')}</option>
              <option value="html-fb">{t('htmlFacetedBrowsing', 'HTML+Faceted Browsing')}</option>
            </select>
          </div>
        </div>
        <div className="mb-3 row align-items-center">
          <label className="col-md-4 col-form-label fw-bold">{t('highlightMode', 'Highlight Mode:')}</label>
          <div className="col-md-8">
            <select
              className="form-select form-select-sm"
              style={{ maxWidth: 200, display: 'inline-block' }}
              value={get('ext.osds.super_links.highlight') || 'first'}
              onChange={e => set('ext.osds.super_links.highlight', e.target.value)}
              aria-label={t('highlightMode', 'Highlight Mode')}
              aria-required="false"
            >
              <option value="first">{t('firstMatch', 'First match')}</option>
              <option value="all">{t('allMatches', 'All matches')}</option>
            </select>
          </div>
        </div>
        <div className="mb-3 row align-items-center">
          <label className="col-md-4 col-form-label fw-bold">{t('retries', 'Retries:')}</label>
          <div className="col-md-8">
            <input
              className="form-control form-control-sm d-inline-block"
              style={{ maxWidth: 60, display: 'inline-block' }}
              type="number"
              value={get('ext.osds.super_links.retries') || 3}
              onChange={e => set('ext.osds.super_links.retries', e.target.value)}
              aria-label={t('retries', 'Retries')}
              aria-required="false"
            />
          </div>
        </div>
        <div className="mb-3 row align-items-center">
          <label className="col-md-4 col-form-label fw-bold">{t('retriesTimeout', 'Retries Timeout (s):')}</label>
          <div className="col-md-8">
            <input
              className="form-control form-control-sm d-inline-block"
              style={{ maxWidth: 60, display: 'inline-block' }}
              type="number"
              value={get('ext.osds.super_links.retries_timeout') || 2}
              onChange={e => set('ext.osds.super_links.retries_timeout', e.target.value)}
              aria-label={t('retriesTimeout', 'Retries Timeout')}
              aria-required="false"
            />
          </div>
        </div>
        <div className="mb-3 row">
          ...
          <div className="col-12">
            <textarea
              ...
              value={get('ext.osds.super_links.query') || ''}
